...
switch ($method) {
        ...
        */
        ...
        if ($res = $production->getProductList()) {
            $requestResult['Products']['Product'][] = [
                ...
                "ProductCreditCost"   => $ds['product_credit_cost']
            ];
        }
        break;
    ...
    case 'createProduct':
        ...
        */
        ...
        break;
    case 'addSubProduct':
        if ($res = $production->addSubProduct($_POST)) {
            $requestResult = ["ID" => $res];
        }
        break;
    ...
    case 'getOpenMotifList':
        ...
        */
        ...
        break;
    case 'getOpenJobs':
        $requestResult = ["OpenJobs" => ["Jobs" => [
                "Job" => []
            ]]];
        if ($res = $production->getOpenJobs(false)) 
		{
            while ($ds = mysqli_fetch_array($res)) 
			{
                $requestResult['OpenJobs']['Jobs']['Job'][] = [
                    "ProductionID"         => $ds['production_production_id'],
                    "FilmName"             => $ds['name'],
                    "ProductionCreationTime"         => strtotime($ds['create_ts']),
                    "ProductionUpdateTime"           => strtotime($ds['production_update_ts']),
                    "ProductionStatus"     => $ds['production_status'],
                    "ProductionErrorCode"  => $ds['production_error_code'],
					"ProductionRendererId" => $ds['rendermachine_id'],
                    "AccountID"            => $ds['account_id'],
                    "IndicativeID"         => $ds['indicative'],
                    "AbdicativeID"         => $ds['abdicative'],
                    "AudioID"              => $ds['audio_id'],
                    "SpecialIntroMusic"    => $ds['special_intro_music'],
                    "IsPreview"            => $ds['is_preview'],
                    "FilmID"               => $ds['film_id'],
		...
		if ($res = $production->getOpenJobs(true)) 
		{
            while ($ds = mysqli_fetch_array($res)) 
			{
                $requestResult['OpenJobs']['Jobs']['Job'][] = [
		...
		if ($res = $production->getOpenDicativeJobs($_POST)) 
		{			
			while ($ds = mysqli_fetch_array($res)) 
			{
				$requestResult['OpenJobs']['Dicatives']['Dicative'][] = [
					"JobID"               => $ds['job_job_id'],
					"Status"           => 10,
    ...
    case 'checkProductionStatusAdv':
        $requestResult = ["Films" => []];
        if ($res = $production->checkProductionStatusAdv($_POST)) {
            while ($ds = mysqli_fetch_array($res)) {
                $requestResult["Films"][] = [
                    "id"     => $ds['film_id'],
                    "status" => $ds['job_status'],
    ...
    case 'adjustHistory':
        ...
        break;
    case 'healHistory':
        $production->healHistory();
        $requestResult = ["DONE" => 1];
        break;
    case 'getCodecByFilmId':
        $requestResult = ["Codecs" => $production->getCodecByFilmId($_POST)];
        break;
    case 'getCodecTypes':
        $requestResult = ["Codecs" => $production->getCodecs()];
        break;
    case 'updateProductStatistics':
         $production->updateProductStatistics($_POST);
         $requestResult = ["DONE" => 1];
         break;
	case 'sendHeartbeat':
        if ($res = $production->sendHeartbeat($_POST)) 
        {
			$license_status = $res[0];
			$machine_status = $res[1];
            $result = [];
            while ($ds = mysqli_fetch_array($machine_status)) 
            {
                $result[] = [
                    "id"     => $ds['id'],
                    "machinename" => $ds['machinename'],
                    "is_active" => $ds['is_active'],
                    "priority" => $ds['priority'],
                    "status" => $ds['status'],
                    "message" => $ds['message'],
					"force_active" => $ds['force_active']
                ];
            }
