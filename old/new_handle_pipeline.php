 ...
 */
  ...
  while ($job = mysql_fetch_object($result))
  {
	...
	$query2 = "SELECT * FROM jobQueue WHERE productID = ".$jobID." ORDER BY jobCounter";
...
//insert production data into film database
		$query = "INSERT INTO films (name, codecFormats, products, sizeKB, creationDate, duration, audio, watermarkID, preview, url, ownerID, jobID) ";
		$query .="VALUES ('".$name."', '".$job->productionInfo."', '".$job->productIDs."', '".implode("+",$filesize)."', '".time()."', ".$duration.", 0, 0, 2, '".$companyID."/".$ownerID."', ".$ownerID.", ".$jobID.")";
		mysql_query($query);	
		writelog($query);
