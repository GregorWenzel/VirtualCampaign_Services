...
*/ 
   ...
   $cmd = '"'.$im_path.'/convert.exe" "'.$UPLOADDIR.$name.'.jpg" -resize '.$aspect*$NEWHEIGHT_L."x".$NEWHEIGHT_L.' "'.$UPLOADDIR.$name.'_bigthumb'.$stamp.'.jpg"';

   writelog($cmd);
   writelog(syscall($cmd));

  // unlink($UPLOADDIR.$rnd.$extension);
   writelog("<Result><Name>".$name."</Name><Animated>0</Animated><nFrames>1</nFrames><Aspect>".$aspect."</Aspect><Time>".$stamp."</Time></Result>");
   print("<Result><Name>".$name."</Name><Animated>0</Animated><nFrames>1</nFrames><Aspect>".$aspect."</Aspect><Time>".$stamp."</Time></Result>");
